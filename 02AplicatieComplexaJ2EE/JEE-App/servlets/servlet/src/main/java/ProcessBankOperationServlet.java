...
protected void doPost(HttpServletRequest request,
    ...
    // se incearca preluarea bean-ului folosind obiectul HttpSession, care pastreaza o sesiune HTTP intre client si serverBankAccountBeanRemote bankAccount;
    BankAccountBeanRemote bankAccount = (BankAccountBeanRemote) request.getSession().getAttribute("bankAccountBean");
    // daca nu exista nimic pastrat in sesiunea HTTP, inseamnacabean-ul se preia prin JNDI lookup
    if (bankAccount == null) {
        try {
            InitialContext ctx = new InitialContext();
            bankAccount = (BankAccountBeanRemote)
                    ctx.lookup("bankaccount#interfaces.BankAccountBeanRemote");
    // dupa preluarea bean-ului prin JNDI, obiectulsestocheaza in sesiune pentru a fi refolosit ulterior
    ...
    String message = "";
    // in functie de operatia selectata de client, se apeleazametoda corespunzatoare din obiectul remote
    ...
    message += "<br /><a href='./'>Inapoi la meniul principal</a>";// dupa construirea mesajului raspuns, acesta este trimiscasi continut HTML inapoi la clientul apelant
