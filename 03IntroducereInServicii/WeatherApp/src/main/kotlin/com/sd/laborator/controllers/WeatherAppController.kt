...
class WeatherAppController {
    @Autowired
    private lateinit var orchestratorService: OrchestratorService

    @RequestMapping("/getforecast/{location}", method = [RequestMethod.GET])
    @ResponseBody
    fun getForecast(@PathVariable location: String): ResponseEntity<String> {
        val result = orchestratorService.getWeatherForecast(location)
    ...
    fun getForecast(@PathVariable location: String): ResponseEntity<String> {
        ...
        if(coordinates == null) {
            return ResponseEntity.badRequest().body("Could not find data for location \"$location\"!")
        }
    ...
    */
    /*
        ...
        val locationId = locationSearchService.getLocationId(location)// dacă locaţia nu a fost găsită, răspunsul va fi corespunzător
        if (locationId == -1) {
